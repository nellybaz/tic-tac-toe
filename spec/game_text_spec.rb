...
RSpec.describe GameText do
  it 'prints correct game turn text' do
    player1 = HumanPlayer.random1
    player2 = ComputerPlayer.random2
    ...
    GameText.player_turn_text(player1, HumanPlayer.random2)

    expect(GameText).to receive(:puts).with("Player 2's turn [e.g 1,4,7,0]")
    GameText.player_turn_text(HumanPlayer.random2, player1)
  end
  ...
  it 'prints correct game winner text' do
    ...
    GameText.game_winner_text(player1, HumanPlayer.random2)

    expect(GameText).to receive(:puts).with('Player 2 won')
    GameText.game_winner_text(HumanPlayer.random2, player1)
  end
